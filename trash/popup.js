import { CONSUMER_KEY, CONSUMER_SECRET } from './conf/conf';
...
http.createServer(function (request, response) {
    oa.getOAuthRequestToken(function (error, oAuthToken, oAuthTokenSecret, results) {
        ...
        var authURL = 'http://fantasysports.yahooapis.com/fantasy/v2/' +
            ...
            '/callback': function (request, response) {
                ...
                var getOAuthRequestTokenCallback = function (error, oAuthAccessToken,
                    ...
                    oa.get('https://api.twitter.com/1.1/account/verify_credentials.json',
                           ...
                           function (error, yahooResponseData, result) {
                               if (error) {
                                   console.log(error)
                               ...
                               try {
                                   console.log(JSON.parse(yahooResponseData));
                               } catch (parseError) {
                                   console.log(parseError);
                               }
                               console.log(yahooResponseData);
                               response.end(yahooResponseData);
                           });
                };
